    ...
    try {
        ...
        const imagesData = await Promise.all(
            folders.data.map(async (folder) => {
                const sanitizedFolderName = folder.name.replace(/[^a-zA-Z0-9-_]/g, ' '); // Sanitize folder name
                const { data: images, error } = await supabase.storage.from('Gallery').list(`items/${sanitizedFolderName}`);
                if (error) {
                    console.error(`Failed to fetch images for folder ${sanitizedFolderName}:`, error.message);
                    return { folder: sanitizedFolderName, images: [] };
                }
                return { folder: sanitizedFolderName, images };
            })
        );
        ...
        return { productData: [], productInventory: [], sizeData: [], images: [], error: 'Error fetching data' };
    }
};
...
export const getImages = async (
    ...
    const sanitizedItemName = itemName.trim().toLowerCase().replace(/[^a-z0-9-_ ]/g, '');
